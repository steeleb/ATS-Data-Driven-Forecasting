...
standardize_version = '2024-12-01'
...
```{python}
file_path = os.path.expanduser("/Users/steeleb/OneDrive - Colostate/NASA-Northern/data/NN_train_val_test/SMR_forecast/met_3h_complete/")
...
```{python}
rh_cols = [col for col in tv_trans.columns if "rh" in col and not col.split("_m")[-1].isdigit()]

for idx, c in enumerate(rh_cols):
  print(c)
  print(skew(tv_trans[c]))

```

Min and max need to be transformed.

```{python}
rh_cols = [col for col in tv_trans.columns if "rh" in col and not 'ave' in col and not col.split("_m")[-1].isdigit()]
...
Minimum can be transformed using log transformation, max needs Box-Cox
transformation.

```{python}
bc_rh = [col for col in tv_trans.columns if "rh" in col and 'max' in col and not col.split("_m")[-1].isdigit()]
log_sr = [col for col in tv_trans.columns if "rh" in col and 'min' in col and not col.split("_m")[-1].isdigit()]

for idx, c in enumerate(log_sr):
  print(f'transforming {c}')
  tv_trans[c] = np.log(tv_trans[c])
  handle_tracking = pd.concat([handle_tracking, 
    pd.DataFrame({'column': [c], 
...
```{python}
file_name = "data_handle_tracking_t2022_v" + standardize_version + ".csv"
# join with file path
