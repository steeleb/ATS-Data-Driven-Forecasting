...
```{r}
t2023 <- read_csv('data/NN_train_val_test/SMR_autoNN_daily/trainval_t2023_v2024-05-09.csv')
...
```{r}
col_names = names(t2023_reduce)

yesterday <- t2023_reduce %>% 
...
```{r}
# collate data
t2023_reduced <- reduce(list(yesterday_reduced, pump_reduced, temp, wind_reduced, solrad, precip, NF_reduced, chip_reduced),
                        full_join)

reduce_names = names(t2023_reduced)
...
```{r}
test <- read_csv(file.path(og, paste0('t2023_standardized_v',
                                      upstream_version, 
                                      '.csv')))
...
write_csv(test_reduced, file.path(fp, paste0('t2023_reduced_standardized_v', reduced_version, '.csv')))
```
